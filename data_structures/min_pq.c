...
static void percolate_down(item_t *heap, node_t node);
static void heap_swap(item_t *heap, node_t i, node_t j);

min_pq_t *min_pq_create(void){
    min_pq_t *q = malloc(sizeof(min_pq_t));
    if (q){
        q->size = 0;
        q->capacity = MIN_CAPACITY;
        q->heap = malloc(sizeof(item_t)*q->capacity);
        if (!q->heap) {
            free(q);
            q = NULL;
        }
...
const void min_pq_min(min_pq_t *q){
    if (min_pq_size(q) == 0){
        return NULL;
    }
...
void update_capacity(min_pq_t *q){
    ...
    else if (q->size + 1 < old_cap/4 && old_cap >= 2*MIN_CAPACITY){
        q->capacity /= 2;
    }
...
node_t min_child(item_t *heap, node_t node, size_t size){
    node_t left_child = LEFT_CHILD(node);
    node_t right_child = RIGHT_CHILD(node);
    if (right_child > size || heap[left_child].priority <= heap[right_child].priority){
        return left_child;
    }
    return right_child;
}
